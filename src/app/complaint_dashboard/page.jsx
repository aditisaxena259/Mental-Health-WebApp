...
function ComplaintDashboardInner() {
  ...
  // Keep initial metrics fetch (optional), but we'll derive on-screen stats from current filters below
  useEffect(() => {
    const token = getToken();
    if (!token) return;
    Promise.all([
      api("/metrics/status-summary", { method: "GET" }, token),
      api("/metrics/resolution-rate", { method: "GET" }, token),
      api("/metrics/pending-count", { method: "GET" }, token),
    ])
      .then(([summary, rate, pending]) => {
        const next = {
  ...
  useEffect(() => {
    ...
    const loadComplaints = async () => {
      ...
      setListError("");
      try {
        // Try admin list endpoint first
        const resp = await api("/admin/complaints", { method: "GET" }, token);
        ...
        try {
          // Fallback to generic list endpoint if available
          const resp2 = await api("/complaints", { method: "GET" }, token);
  ...
  // Normalize backend status to our internal values
  const normalizeStatus = (s) => {
    if (!s) return s;
    if (s === "pending") return "open";
    if (s === "in-review") return "inprogress";
    return s; // open | inprogress | resolved
  };

  // Map backend complaint type to UI category id
  const mapTypeToCategoryId = (t) => {
    if (!t) return t;
    if (t === "Lost and Found") return "lost-found";
    if (t === "Other Issues") return "other";
    return t; // roommate | plumbing | cleanliness | electricity
  };
  ...
  const filteredComplaints = complaints
    .filter(
      (c) =>
        ...
        mapTypeToCategoryId(c.type) === selectedCategory
    )
    .filter(
      (c) =>
  ...
  // Derive on-screen stats from filteredComplaints to reflect current category/filters/search
  const derivedStats = useMemo(() => {
    const total = filteredComplaints.length;
    const resolved = filteredComplaints.filter(
      (c) => normalizeStatus(c.status) === "resolved"
    ).length;
    const inReview = filteredComplaints.filter(
    ...
    return {
      ...
      inReview,
      resolutionRate,
      avgResolutionTime: stats.avgResolutionTime, // still not provided
    };
  }, [filteredComplaints, stats.avgResolutionTime]);
                ...
                onClick={() => {
                  ...
                  const rows = filteredComplaints.map((c) => ({
                    ...
                    student:
                      ...
                      c.student?.name ||
                      "",
                    room: c.student?.roomNo || c.student?.room_no || "",
                    type: c.type,
                  ...
                  const stamp = new Date()
                    ...
                    .replace(/[:T]/g, "-");
                  a.href = url;
                  a.download = `complaints-export-${selectedCategory}-${statusFilter}-${stamp}.csv`;
                ...
                <CardContent className="pb-2">
                  ...
                  <div className="p-4 bg-blue-50 rounded-lg transition-transform hover:scale-105">
                    ...
                    <p className="text-3xl font-bold text-blue-800">
                      {derivedStats.avgResolutionTime}
                    </p>
              ...
              <div className="flex gap-2 md:gap-3">
                ...
                <div className="hidden md:block">
                  <Popover>
                    ...
                    <PopoverContent className="w-80 p-0 rounded-md shadow-lg border-gray-200 bg-white">
                ...
                <div className="bg-red-50 text-red-700 border border-red-200 rounded-md p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      variant="outline"
                      onClick={() => {
                        // Trigger reload by re-running effect logic
                        const token = getToken();
                        if (!token) return;
                        (async () => {
                          setLoadingList(true);
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                        <div className="flex items-start gap-3">
                          <div className="bg-indigo-100 text-indigo-700 p-2 rounded-md">
                            <CategoryIcon
                              category={mapTypeToCategoryId(complaint.type)}
                            />
                          </div>
                          <div>
